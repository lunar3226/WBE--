...
const MAP_MIN_RENDER_SCALE = 2;    // supersample on standard-density displays

function getMapRenderScale() {
  const deviceScale = window.devicePixelRatio || 1;
  return Math.max(MAP_MIN_RENDER_SCALE, deviceScale);
}
...
function updateMap(idx) {
  const isSameFrame = idx === mapIdx && mapAnimProgress >= 1;
